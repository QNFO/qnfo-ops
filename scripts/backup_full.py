...
def main():
    ...
    for label, script in (("config", CFG), ("agent.db", DB)):
        t0 = time.time()
        print(f"[backup_full] running {label} ({os.path.basename(script)}) ...", flush=True)
        try:
...
" + r.stderr) if r.stderr else "")
            ...
            print(f"[backup_full] {label} exit={r.returncode} in {int(time.time()-t0)}s", flush=True)
            # surface tail of child output for the log
            tail = "
